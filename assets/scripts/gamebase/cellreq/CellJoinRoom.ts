...
class CellJoinRoom extends CellBase {
    ...
    start(body:any, timeOutTime:number): boolean {
        ...
        if (!super.start(body, Stype.S_TYPE.GameHoodle, GameHoodleProto.XY_ID.eJoinRoomReq, timeOutTime)) {
            return false;
        }
        return true;
    }
    ...
    onMsgReceive(stype:number, ctype:number, body:any) {
        if (stype != Stype.S_TYPE.GameHoodle || ctype != GameHoodleProto.XY_ID.eJoinRoomRes){
            return;
        }
        this.success(body);
    }
