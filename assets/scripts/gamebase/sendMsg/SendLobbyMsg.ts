...
export default class SendLobbyMsg {
    ...
    static send_join_room(roomid: string) {
        ...
        SendLobbyMsg.send(LobbyProto.XY_ID.REQ_JOINROOM, { roomid: roomid });
    }

    //退出房间
    static send_exit_room() {
        SendLobbyMsg.send(LobbyProto.XY_ID.REQ_EXITROOM);
    }

    //解散房间
    static send_dessolve_room() {
        SendLobbyMsg.send(LobbyProto.XY_ID.REQ_DESSOLVEROOM);
    }

    //是否创建了房间
    static send_get_room_status() {
        SendLobbyMsg.send(LobbyProto.XY_ID.REQ_ROOMSTATUS);
    }
    ...
    static send_get_ugame_info(){
        SendLobbyMsg.send(LobbyProto.XY_ID.REQ_GAMEINFO);
    }
